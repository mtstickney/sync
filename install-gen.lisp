...
(definstaller :cmax-4.3-db
  ...
  (:pre-effects :ensure-db-machine
                ...
                (:add-df "df/delta2.df")
                (:msg "Done.~%")
                (:msg "Initializing tables...")
                ;; (:run-procedure "init/mailingevent.r" :src)
                ;; (:run-procedure "init/tags.r" :src)
                (:msg "Done.~%")
                (:funcall (lambda ()
                            (when *stopped-dbs*
...
(definstaller :cmax-4.3-code
  ...
  (:probes (*src-dir* #P"src/")
           ...
           (*res-dir* #P"res/")
           (*file-callback* (lambda (def)
                              (declare (ignore def))
                              (let ((i 0))
                                (lambda (item)
                                  (declare (ignore item))
                                  ;; Print a dot every 5 files
                                  (when (= (mod (incf i) 5) 0)
                                    (format t ".")))))))
  (:tree (:+ #P"./"))
  (:pre-effects (:msg "Copying files...."))
  (:post-effects (:msg "Done.~%")))
