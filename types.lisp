...
(defmethod cffi:translate-from-foreign (value (type variant))
  (cond
    ...
    (t (ecase (variant-data-type value)
         (:empty (error "Variant is uninitialized, cannot convert from foreign."))
         (:null nil)
         (:int16 (variant-slot-value value 'ival))
         (:int32 (variant-slot-value value 'lval))
         (:float (variant-slot-value value 'fltval))
         (:double-float (variant-slot-value 'dblval))
         (:currency (variant-slot-value 'cyval))
         (:bstr (variant-slot-value value 'bstrval))
         (:idispatch (variant-slot-value value 'pointer))
         (:error (variant-slot-value value 'hresultval))
         (:bool (variant-slot-value value ))
         ))
    )
  )
